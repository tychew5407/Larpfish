...
void make_null_move(board *b, zobrist_board game_history[]) {
    // Update board struct and game history
    update_move_counters(b, game_history, NO_PIECE, b->play_side, QUIET);
    update_playside(b, game_history, b->play_side);
    update_ep(b, game_history, 0, b->play_side, QUIET);
    update_castling(b, game_history, 0, NO_PIECE, b->play_side);
...
void unmake_null_move(board *b, zobrist_board game_history[]) {
    ...
    move_stack_index --;
    game_history[b->halfmove_clock] = move_stack[move_stack_index].zobrist_pos;
    b->play_side = b->play_side ^ 1;
    b->ep_square = move_stack[move_stack_index].ep_square;
    b->castling = move_stack[move_stack_index].castling;
    b->halfmove_clock = move_stack[move_stack_index].halfmove_clock;
    b->fullmove_counter -= b->play_side;
}
